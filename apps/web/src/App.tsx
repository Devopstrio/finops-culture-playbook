...
import { ShieldCheck, Activity, Database, Users, TrendingUp, BarChart4, Settings, LayoutDashboard, Globe, Zap, Box, Anchor, Share2, Server, Repeat, AlertTriangle, Layers, Wallet, Target, PieChart, Info, BookOpen, MessageSquare, Terminal } from 'lucide-react';
      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100 font-sans">
        ...
        <aside className="w-72 bg-slate-900/40 backdrop-blur-3xl border-r border-slate-800 flex flex-col p-8 fixed h-full shadow-2xl">
          <div className="flex items-center gap-4 mb-12">
            ...
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent">FinOpsPlay</span>
          </div>
          
          <nav className="flex-1 space-y-2">
            <NavItem to="/" icon={<LayoutDashboard size={20} />} label="Executive Dashboard" active />
            <NavItem to="/costs" icon={<Wallet size={20} />} label="Cost Insights" />
            <NavItem to="/forecast" icon={<Target size={20} />} label="Forecasting" />
            <NavItem to="/governance" icon={<ShieldCheck size={20} />} label="Governance" />
            ...
            <NavItem to="/playbooks" icon={<BookOpen size={20} />} label="Playbooks" />
            <NavItem to="/nudges" icon={<MessageSquare size={20} />} label="Automated Nudges" />
          </nav>
        ...
        <main className="flex-1 ml-72">
          <header className="h-20 border-b border-slate-800 flex items-center justify-between px-10 bg-slate-950/50 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-2 text-slate-400 text-sm font-medium uppercase tracking-widest">
              <span>Industrial Cloud Economics</span>
              <span>/</span>
              <span className="text-white font-bold">FinOps Culture Playbook</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-sm font-bold text-white">FinOps Leader</p>
                <p className="text-[10px] text-indigo-400 uppercase tracking-widest font-black">Strategic Access</p>
              </div>
